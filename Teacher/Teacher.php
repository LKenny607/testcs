...
// Teacher.php lives in /CSProject/Teacher, but StudentLogin.php lives in
// /CSProject/Student — so the join link has to cross into the sibling folder.
// The QR encodes the current game_code as a query param, so StudentLogin.php
// can verify a scanned code belongs to a live, running game before joining.
$protocol   = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
$teacherDir  = rtrim(dirname($_SERVER['PHP_SELF']), '/'); // .../CSProject/Teacher
$projectRoot = dirname($teacherDir);                      // .../CSProject
$studentBaseUrl = $protocol . '://' . $_SERVER['HTTP_HOST'] . $projectRoot . '/Student';

$canJoin = $gameSession && $gameSession['game_status'] === 'Running';
$joinUrl = $canJoin
    ? $studentBaseUrl . '/StudentLogin.php?code=' . urlencode($gameSession['game_code'])
    : null;
$qrUrl = $joinUrl
    ? 'https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=' . urlencode($joinUrl)
    : null;
?>
<!DOCTYPE html>
<html lang="en">
...
<div class="header">
    ...
    <div class="nav-btns">
        <a href="../Student/student-dashboard.php" class="btn outline">Exit</a>
    </div>
</div>
            ...
            <div class="panel">
                ...
                <div class="qr-card">
                    ...
                    <div style="flex:1; min-width:0;">
                        <p style="font-size:13px; color:var(--text-muted); margin-bottom:6px;">
                            Students tap <strong>Enter Game</strong> on their dashboard and scan this code
                ...
                <?php else: ?>
                    <p style="color:var(--text-muted); font-size:13px;">
                        Start a game above to generate a join QR code for students.
                    </p>
                <?php endif; ?>
            </div>
